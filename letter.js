...
const SKILLS = [
  ...
  { n: 'C# / .NET', lvl: 1, kw: ['c#', 'c sharp', 'шарп', '.net', 'asp'], proof: 'писала на C# приложения и игры, четыре года преподавала C# детям' },
  ...
  { n: 'Обучение / менторство', lvl: 2, kw: ['обучен', 'ментор', 'наставни', 'преподав', 'edtech', 'документаци', 'коммуникаб', 'команд'], proof: 'четыре года преподавала программирование, писала методические материалы и вела проектные работы учеников' }
];
...
function buildLetter(a) {
  ...
  const intro = gamedev
    ? `Меня зовут Анжелина, откликаюсь на вакансию «${role}»${compIn}. Пишу на C#, C++ и Python, разрабатывала игры и визуальные новеллы на Unity (C#) и графические приложения на C++ с SFML, а четыре года преподавала программирование на этих языках — то есть знаю их достаточно глубоко, чтобы объяснять другим.`
  ...
  const teacher = $('v-teacher').checked
    ? `\n\nДо разработки я четыре года преподавала программирование в двух школах: вела C#/Unity, C++, Python и веб, писала методические материалы и сопровождала проектные работы учеников. Это дало мне то, что редко встречается у разработчиков моего уровня: умение внятно объяснять решения на ревью, писать документацию, которую действительно читают, и смотреть на интерфейс глазами человека, который видит его впервые.`
    : '';
  ...
  if (tone === 'short') {
    return `${greet}\n\n${intro}\n\n${match}${gaps}\n\nПортфолио: github.com/Evil-Pinguin — проекты задеплоены и открываются по ссылке.${vision}${feedback}${outro}`;
  }
  if (tone === 'warm') {
    return `${greet}\n\n${intro} Вакансия${compTo ? ' у вас' : ''} откликнулась мне тем, что здесь важен не просто «закрытый тикет», а продукт, которым приятно пользоваться — я как раз про это.\n\n${match}${gaps}${portfolio}${teacher}${vision}${feedback}${outro}`;
  }
  return `${greet}\n\n${intro}\n\n${match}${gaps}${portfolio}${teacher}${vision}${feedback}${outro}`;
}
