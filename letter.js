...
function buildLetter(a) {
  ...
  const analyst = $('v-analyst') && $('v-analyst').checked;

  const intro = analyst
    ? `Меня зовут Анжелина, откликаюсь на позицию «${role}»${compIn}. Последние 4 года я выстраивала систему оценивания результатов для 200+ учеников в двух образовательных организациях в Якутске: проектировала методологию, собирала и анализировала данные о прогрессе, корректировала подход по итогам анализа. Также самостоятельно строила инструменты для сбора и обработки данных на SQL и Python, в том числе платформу тестирования с автоматическим подсчётом и агрегацией результатов.`
    : gamedev
    ? `Меня зовут Анжелина, откликаюсь на вакансию «${role}»${compIn}. Разрабатываю игры на Unity (C#) и C++/SFML полным циклом: игровая логика, сцены, интерфейс. Спрайты и анимацию рисую сама в Krita, так что закрываю связку программист плюс 2D-художник. Делала визуальные новеллы со сценарной структурой и ветвлением диалогов. Четыре года преподавала Unity, C# и Roblox Studio, обучила больше 200 учеников геймдев-направлениям.`
    : `Здравствуйте, меня зовут Анжелина. Откликаюсь на вакансию «${role}»${compIn}. Я frontend-разработчик: пишу на React и TypeScript, собираю проекты на Vite и довожу их до рабочего состояния в проде - от макета в Figma до деплоя на Vercel с автосборкой.`;
  ...
  const match = proofs.length
    ...
    : (gamedev
      ? `Из релевантного: разработка игр на Unity (C#) и C++ (SFML), игровая логика и UI, 2D-графика и анимация в Krita, визуальные новеллы, Roblox Studio.`
  ...
  const gaps = a.miss.length
    ? `\n\nПро пробелы скажу честно: с ${a.miss.map(s => s.n).join(' и ')} в проде я пока не работала. Учусь я быстро: за последний год сама разобралась с TypeScript, serverless-функциями и базой данных. Думаю, закрою это к старту или в первые недели работы.`
    : '';

  const portfolio = analyst
    ? `\n\nЧто можно посмотреть:\n- EduQuiz Pro - платформа тестирования со сбором и подсчётом результатов: my-edu-quiz.vercel.app\n- LexiCard - приложение с метриками обучения, точностью ответов и статистикой прогресса: github.com/Evil-Pinguin/lexicard`
    : gamedev
    ? `\n\nЧто можно посмотреть:\n- Игры и визуальные новеллы на Unity (C#) и C++ (SFML): игровая логика, интерфейс, спрайты и анимация мои.\n- LexiCard - приложение с геймификацией: таймер, звуковая обратная связь, повтор ошибок, AI-генерация контента. github.com/Evil-Pinguin/lexicard\n- Aurora - голосовой помощник для автоматизации работы за компьютером.`
  ...
  const personal = `\n\nПару слов о себе как о сотруднике. Новые задачи и технологии подхватываю на лету, начатое довожу до конца и за результат отвечаю. Несколько лет волонтёрю на крупных мероприятиях республики - международных играх «Дети Азии», фестивале «Муус устар» с самого первого сезона и «Играх Манчаары». Это командная работа там, где подвести нельзя, а решения принимать надо сразу.`;

  const visionAnalyst = `\n\nМне интересно перейти от образовательных данных к продуктовым исследованиям. Я вижу себя в этой роли: умею поставить измеримую гипотезу, собрать данные, довести анализ до практического вывода и защитить его перед командой. Живу в Якутске, релокация не требуется.`;
  ...
  const reloc = (analyst || gamedev) ? ' Живу в Якутске, релокация не требуется.' : ($('v-relocate').checked ? ' Готова к релокации и к работе по московскому времени.' : ' Работаю удалённо, живу в Якутске, к работе по московскому времени готова.');
