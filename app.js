...
const DEFAULT = {
  ...
  tg: '',
  photo: '',
  about: 'Беру продукт и довожу до работающего результата, без передачи между тремя специалистами. Разработчик широкого профиля, 6 лет в IT и педагогике. Пишу на пяти языках: C#, C++, Python, JavaScript и TypeScript. Делала и игры на Unity, и приложения на SFML, и веб-сервисы с серверной частью и базой данных. За 4 года преподавания обучила больше 200 учеников, методики писала сама. Отдельно вытягиваю визуальную часть: интерфейсы в Figma, графика и анимация в Krita, видеомонтаж. По сути веду продукт целиком: придумать, нарисовать, запрограммировать, протестировать и выпустить, а потом объяснить команде, как оно устроено. Новое схватываю быстро, начатое довожу до конца - это же подтверждают годы волонтёрства на крупных мероприятиях республики.',
  achievements: [
    { text: 'Обучила больше 200 учеников программированию за 4 года в двух школах. Методики и учебные материалы писала сама, десятки проектных работ довела до защиты.' },
    ...
    { text: 'Сделала и выпустила 4 приложения полного цикла: архитектура, интерфейс, серверная часть, база данных, тесты и автодеплой. Все работают онлайн, можно открыть и посмотреть.' },
    { text: 'Подключила нейросеть через serverless-функцию: ключи спрятаны в окружении, ответ модели проверяется и приводится к нужному формату.' },
    { text: 'Закрываю то, что обычно делят на троих: дизайн интерфейса, разработку и документацию. Меньше согласований и переписки внутри команды.' },
    { text: 'Вела проектные работы учеников до научных конференций, среди моих учениц есть победительницы муниципальной олимпиады по биологии и экологии.' },
    { text: 'Волонтёр на международных играх «Дети Азии», фестивале «Муус устар» с самого первого сезона и «Играх Манчаары». Командная работа там, где подвести просто нельзя.' }
  ],
  skills: [
  ...
  projects: [
    {
      ...
      desc: 'Приложение полного цикла: React + TypeScript на клиенте, serverless-функция на сервере, интеграция с LLM (Groq API) для генерации учебных наборов. Архитектура Container/Presentational со строгой типизацией. Два режима проверки, таймер, повтор ошибок по методике интервальных повторений, синтез речи, звуковая обратная связь, тёмная тема, адаптив. Методическая часть построена на моём преподавательском опыте.',
    ...
    },
    {
      ...
      desc: 'Два режима: прохождение теста и редактор вопросов. Управление состоянием через Context API, настраиваемый таймер с адаптивной индикацией, валидация форм, сохранение данных, сбор и подсчёт результатов. Задеплоено с автосборкой.',
    ...
    },
    {
      ...
      desc: 'Разработка игровых приложений: игровая логика, сцены, интерфейс. Спрайты и анимацию рисовала самостоятельно в Krita. Проекты использовались как учебные материалы на занятиях с учениками.',
      tags: 'C#, Unity, C++, SFML, Krita, геймдизайн',
      link: ''
    },
    {
      title: 'Aurora - голосовой помощник для компьютера',
      desc: 'Программа-ассистент в духе Джарвиса: упрощает повседневную работу за компьютером. Управление и запуск задач голосом и командами, автоматизация рутинных действий, продуманный сценарий взаимодействия, чтобы пользователю было понятно без инструкции.',
      tags: 'Python, автоматизация, голосовой интерфейс, UX',
      link: ''
    },
    {
      title: 'Обучающий сайт по Linux',
      desc: 'Учебная платформа по основам Linux: структура курса, подача материала от простого к сложному, практические задания. Совмещены методическая часть и вёрстка, весь контент писала сама.',
      tags: 'HTML, CSS, JavaScript, Linux, методика',
      link: ''
    },
    {
      title: 'Wedding Invite - веб-сервис с формой и базой данных',
      desc: 'React и TypeScript с PostgreSQL через Supabase. Форма сбора ответов с валидацией, асинхронной записью в базу, обработкой ошибок и состояний загрузки.',
  ...
  exp: [
    {
      ...
      meta: '4 года',
      desc: 'Обучила больше 200 учеников. Направления: C# и Unity, C++ (SFML), Python, веб-разработка, Roblox Studio, визуальные новеллы. Писала авторские методики и учебные материалы, смотрела уровень групп по срезам и подправляла программу под результат. Вела проектные работы до защиты и до выступлений на научных конференциях, мои ученицы побеждали на муниципальной олимпиаде по биологии и экологии. Графику и спрайты для учебных игр рисовала сама.'
    },
    {
      title: 'Волонтёр международных и республиканских мероприятий',
      meta: 'Регулярно, несколько лет',
      desc: 'Международные спортивные игры «Дети Азии», фестиваль «Муус устар» - хожу с самого первого сезона, Спортивные игры народов Якутии «Игры Манчаары». Сопровождение участников и гостей, координация на площадках, задачи меняются на ходу и решать их надо сразу. Дисциплина и ответственность там на первом месте.'
    },
    {
      title: 'Разработчик программного обеспечения (проектная работа)',
      meta: '2024 - настоящее время',
      desc: 'Полный цикл разработки веб- и десктоп-приложений: продумываю архитектуру, типизирую код, подключаю внешние и AI-API, пишу serverless-функции, работаю с PostgreSQL. Дальше тесты, настройка CI/CD, выпуск в продакшн и документация.'
    },
    {
      title: 'Педагогический институт - высшее образование',
      meta: 'Диплом',
      desc: 'Педагогика. Методология обучения, проектирование систем оценивания, анализ результатов и статистики успеваемости.'
    }
  ]
};
